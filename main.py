...
def main():
    ...
    messages.append({"role": "system", "content" : "입력된 답변(A)에서 빈도수가 가장 높은 상위 5개의 단어와 각각의 횟수를 출력합니다.\n문맥상 불필요한 표현을 텍스트 길이에 비례해 0 ~ 10개 내의 범위 안에서 출력합니다.\n문맥에 맞지 않는 표현을 대체할 수 있는 표현을 추천합니다.\n주어진 질문에 대한 답변이 문맥상 올바른지, 그리고 교정해야할 언어 습관이 무엇인지 피드백해줍니다. 예시를 들어 피드백을 더 자세히 설명해 줄 수도 있습니다"}),
    messages.append({"role": "system", "content": "다음과 같은 형식으로 출력합니다: '단어 빈도수 상위 5개(각각의 횟수도 포함. 단어마다 줄바꿈)' (한 줄 건너띄기) '문맥상 불필요한 표현(표현마다 줄바꿈)' (한 줄 건너띄기) '대체 표현 추천('불필요한 표현 -> 대체 표현(삭제할 경우 '삭제' 라고 출력)'의 형식으로 출력. 표현마다 줄바꿈)' (한 줄 건너띄기) '피드백(필요 시 예시 제공)'"}),
